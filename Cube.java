...
public class Cube {
	static Slot[][][] cube;
	static String scramble = JOptionPane
			.showInputDialog("What is your scramble?").toUpperCase();
	...
	private static void topCorners() {
		...
		for (int j = 4; j < 8; j++) {
			piece = corners.get(j);
			if (piece.getColor()[1] == cube[1][1][piece.getCoordinates()[2]].getColor()[0]
					&& piece.getColor()[2] == cube[1][piece.getCoordinates()[1]][1].getColor()[0]) {
				pieceSolved = j - 4;
				countSolved++;
			}
		...
		// this means it is an E-perm or an H-perm
			...
			if (cube[2][0][0].getColor()[2] == cube[1][1][0].getColor()[0]
					&& cube[2][0][0].getColor()[1] == cube[1][2][1].getColor()[0]) {
				F();
				Lprime();
				Fprime();
				R();
			...
			} else if (cube[2][0][0].getColor()[1] == cube[1][1][2].getColor()[0]
					&& cube[2][0][0].getColor()[2] == cube[1][2][1].getColor()[0]) {
				// just two e-perms
				F();
				Lprime();
				Fprime();
	...
	private static int[] edgeTurns(int[] numberOfTurns) {
		// Don't worry about this, it's just to help the next methods find which pieces
		// need to go where
		for (int j = 8; j < edges.size(); j++) {
			if (edges.get(j).getColor()[1] == 1) {
				if (cube[1][edges.get(j).getCoordinates()[1]][edges.get(j).getCoordinates()[2]].getColor()[0] == 1) {
					// red on red is fine
					numberOfTurns[j - 8] = 0;
				} else if (cube[1][edges.get(j).getCoordinates()[1]][edges.get(j).getCoordinates()[2]]
						.getColor()[0] == 2) {
					numberOfTurns[j - 8] = 2;
				} else if (cube[1][edges.get(j).getCoordinates()[1]][edges.get(j).getCoordinates()[2]]
						.getColor()[0] == 4) {
	...
	private static void topFace() {
		...
		while (countNonYellows > 0) {
			...
			// either 2, 3, or 4 are not yellow-we want it to get to 3 so we can do one move
			// to solve it.
	...
	private static void middleEdges() {
		...
		for (int a = 0; a < 4; a++) {
			...
			for (int j = 4; j < edges.size(); j++) {
				...
				if (s.getCoordinates()[0] == 1 && (s.getColor()[0] == cube[1][1][s.getCoordinates()[2]].getColor()[0])
						&& (s.getColor()[1] == cube[1][s.getCoordinates()[1]][1].getColor()[0])) {
					s.solved();
				}
				// if its not white and its not yellow, and it's not solved, then it needs
				// solving
				...
				if (needsSolve) {
					...
					// Now any edge if the edge was in the middle layer it is now in the yellow
					// face.
					if (face == 0) {
						// this means it started and ended in the bottom layer, so face was never
						// assigned
						// L face
					...
					// Now, find orientation, then find the correct secondary face (red, orange,
					// green, or blue), then insert the edge!
					...
					// NOTE: The only way to do this is to hard code it, because the pieces don't
					// reload their positions enough
					// This is, if the center directly above the current edge has the same color as
					// the piece's second color
					...
					// At this point, theoretically the middle layer should be done *crossing
					// fingers
			...
			for (int j = 0; j < corners.size(); j++) {
				...
				for (int i = 0; i < s.getColor().length; i++) {
					...
					if (s.getCoordinates()[0] == 0
							&& (s.getColor()[1] == cube[1][1][s.getCoordinates()[2]].getColor()[0])
							&& (s.getColor()[2] == cube[1][s.getCoordinates()[1]][1].getColor()[0])) {
						s.solved();
					}
					if (s.getColor()[i] == 6 && !s.isSolved()) {
						...
						// NOTE: The only way to do this is to hard code it, because the pieces don't
						// reload their positions enough
						// This is, if the centers directly above the current corner has the same color
						// as the pieces second color and third color
						if (face == 0) {
							// this means it started and ended in the bottom layer, so face was never
							// assigned
							// L face
						...
						for (int x = 0; x < 4; x++) {
							// either the first non white color has the color of the front/back or the color
							// of the left/right
		...
		for (int a = 0; a < 4; a++) {
			...
			for (int j = 0; j < edges.size(); j++) {
				...
				for (int i = 0; i < s.getColor().length; i++) {
					...
					// makes sure it's not already solved
					if (s.getCoordinates()[0] == 0 && (s
							.getColor()[1] == cube[1][s.getCoordinates()[1]][s.getCoordinates()[2]].getColor()[0])) {
						s.solved();
					}
					if (s.getColor()[i] == 6 && !s.isSolved()) {
						...
						// because it's essentially the same thing, I will copy/paste the code from
						// before:
						// L face
	...
	private static void scramble() {
		...
		for (int i = 0; i < scramble.length(); i++) {
			...
			if (i < scramble.length() - 1 && (scramble.charAt(i + 1) == '\'' || scramble.charAt(i + 1) == '2')) {
				move = move + scramble.charAt(i + 1);
				i++;
			} else if (Character.isWhitespace(scramble.charAt(i))) {
				continue;
			}
			temp.add(move);
		}
		String[] moves = new String[temp.size()];
	...
	private static void drawLeft() {
		...
		for (int c = 0; c < 3; c++) {
			for (int l = 0; l < 3; l++) {
				...
				// To future me or to other coders I may show this to, basically it goes to the
				// orientation index,
				// and travels forward in the
				// array by the array's length -1 to get to the value
				// which is last in priority
				// we know right and left are last, so these are always last and never in
				// priority, except for centers.
				// Just to protect from out of bounds exceptions, we use modulus of the array's
				// length to wrap back around
				// for example, {6,4,1} where 4 is the orientation, that means 6 is on the left
				// or right side,
				// we go forward by two, which is out of bounds but wraps back around to index =
				// 0, value = 6
				...
				// because of how the cube unfolds. If you imagine it in you mind, you will see
				// why depth
	...
	// after every turn, this allows the lists to have all of the current corners
	// and edges, but centers can't change, so no need to reload that
